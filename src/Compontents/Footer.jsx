  ...
  return (
    <footer className='w-full flex flex-col-reverse items-center md:flex-row md:justify-between md:items-center border-t border-t-gray-300' style={{ padding: "0px 20px" , marginTop:'40px'}}>
      <div className='flex gap-3 items-center'>
        {currentMode == 'light' ?
          <Link to='/' className={`footer-logo ${i18n.language == 'ar' ? 'right-[7%]' : 'left-[7%]'}`}><img src={logoLight} className='transition-all' alt="" width={40} height={40} /></Link> :
          <Link to='/' className={`footer-logo ${i18n.language == 'ar' ? 'right-[7%]' : 'left-[7%]'}`}><img src={logoDark} className='transition-all' width={40} height={40} /> </Link>
        }
